...
fn buildAskModelToolProjection(
    alloc: Allocator,
...
fn runPromptInternal(alloc: Allocator, prompt: []const u8, permission_override: ?PermissionMode, cfg: Config, options: RunOptions) !PromptRunResult {
    ...
    const session_child_capability = if (ctx.writable) |*writable|
        ...
        null;
    var tool_projection = try buildAskModelToolProjection(alloc, ctx.cfg.mode_registry, options.deps.tool_set, ctx.mode_id, .{
...
fn testProcessQueuedPromptChecksExecOnlyTerminal(deps: *const agent_runtime.AgentRuntimeDeps, semantic_presentation: ?agent_runtime.SemanticPresentationSink, _: agent_runtime.LifecycleContext, cfg: agent_runtime.Config, _: worker_runtime.QueuedPrompt) !void {
    ...
    const model_terminal = for (cfg.model_tools) |descriptor| {
        if (std.mem.eql(u8, descriptor.name, "terminal")) break descriptor;
    } else return error.TestExpectedEqual;
    try std.testing.expect(std.mem.find(u8, model_terminal.description, "Run one captured command and return its result.") != null);
    try std.testing.expect(std.mem.find(u8, model_terminal.description, "Use start for commands") == null);
