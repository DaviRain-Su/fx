...
const header =
    ...
    "A listed server is not a reason to use MCP. Use capability_search only when the task clearly needs a capability not already available locally. Pass the natural-language task and an optional exact server alias; the runtime owns routing and catalog traversal. Then select one exact relevant MCP result with mcp_select_tool. Do not guess identities or preload schemas.\n" ++
    "<mcp_servers>\n";
