...
By default, reviews are answered by a language model through your configured connection. The `review_model` setting in `~/.fx/settings.json` (or the `FX_REVIEW_MODEL` environment variable, which wins) changes the reviewer model. Any AI Gateway chat model id works and keeps the same review protocol. Setting it to `typesafeai/jev` routes each review to TypeSafe's Jev evaluation model instead, which returns a typed decision with calibrated probabilities and a confidence score. Jev is reached through the gateway's evaluation endpoint with your existing gateway credential; setting `TYPESAFE_API_KEY` talks to TypeSafe's API directly instead. Without a working reviewer, actions that need review stay unexecuted rather than falling back silently. The review policy, the action under review, and the hold-or-authorize behavior are identical either way.
